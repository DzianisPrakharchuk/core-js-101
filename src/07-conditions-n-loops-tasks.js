...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const current = arr[i];
    const coincidence = arr.filter((el) => el === current);
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${StartSymb}${smaller}, ${bigger}${endSymb}`;
}
...
function getDigitalRoot(num) {
  ...
  const sum = array.reduce((accumulator, currentValue) => accumulator + currentValue);
  if (sum > 9) {
    return sum.toString().split('').map(Number).reduce((accumulator, currentValue) => accumulator + currentValue);
  }
  return sum;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (currentBracket in bracketsMap) {
      stack.push(currentBracket);
    } else {
      const lastOpeningBracket = stack.pop();
      const expectedClosingBracket = bracketsMap[lastOpeningBracket];
...
function getCommonDirectoryPath(paths) {
  ...
  for (let i = 0; i < pathSegments[0].length; i += 1) {
    ...
    if (pathSegments.some((segments) => segments[i] !== segment)) {
      break;
    }
